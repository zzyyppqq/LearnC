...
int simplest_udp_parser(int port)
{
    ...
    int bind_ret = bind(serSocket, (struct sockaddr *)&serAddr, sizeof(serAddr));
    if(bind_ret == -1){
        printf("bind error !");
        return 0;
    }
    ...
    while (1){
        ...
        if (pktsize > 0){
            ...
            if(parse_rtp!=0){
                ...
                fprintf(myout,"write file rtp_data_size: %d, rtp_header_size: %d, payload: %d\n", rtp_data_size, rtp_header_size, payload);

                //Parse MPEGTS
                if(parse_mpegts!=0&&payload==33){
                    MPEGTS_FIXED_HEADER mpegts_header;
                    for(int i=0;i<rtp_data_size;i=i+188){
                        fprintf(myout,"rtp_data[%d]: %d, value: %d\n", i, rtp_data[i], 0x47);
                        if(rtp_data[i]!=0x47)
                            break;
                        //MPEGTS Header
